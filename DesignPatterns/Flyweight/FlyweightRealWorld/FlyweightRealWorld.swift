...
class Cell {
    ...
    func update(with animal: Animal) {
        ...
        debugPrint("Cell: Updating an appearance of a \(type)-cell: \(photos)\n")
    }
}
...
class AppearanceFactory {
    ...
    static func appearance(for key: AnimalType) -> Appearance {
        ...
        guard cache[key] == nil else {
            debugPrint("AppearanceFactory: Reusing an existing \(key.rawValue)-appearance.")
            return cache[key]!
        }

        debugPrint("AppearanceFactory: Can't find a cached \(key.rawValue)-object, creating a new one.")
...
class FlyweightRealWorld: XCTestCase {
    ...
    func testFlyweightRealWorld() {
        ...
        debugPrint("Client: I created a number of objects to display")

        /// Displaying objects for the 1-st time.
        debugPrint("Client: Let's show animals for the 1st time\n")
