...
class Subject {
    ...
    func attach(_ observer: Observer) {
        debugPrint("Subject: Attached an observer.\n")
    ...
    func detach(_ observer: Observer) {
        if let idx = observers.firstIndex(where: { $0 === observer }) {
            ...
            debugPrint("Subject: Detached an observer.\n")
        }
    }

    /// Запуск обновления в каждом подписчике.
    func notify() {
        debugPrint("Subject: Notifying observers...\n")
    ...
    func someBusinessLogic() {
        ...
        debugPrint("Subject: My state has just changed to: \(state)\n")
        notify()
    }
}
...
class ConcreteObserverA: Observer {
    ...
    func update(subject: Subject) {
        if subject.state < 3 {
            debugPrint("ConcreteObserverA: Reacted to the event.\n")
        }
    }
}
...
class ConcreteObserverB: Observer {
    ...
    func update(subject: Subject) {
        if subject.state >= 3 {
            debugPrint("ConcreteObserverB: Reacted to the event.\n")
        }
    }
}
