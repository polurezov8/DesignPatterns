...
class Context {
    ...
    func doSomeBusinessLogic() {
        debugPrint("Context: Sorting data using the strategy (not sure how it'll do it)\n")

        let result = strategy.doAlgorithm(["a", "b", "c", "d", "e"])
        debugPrint(result.joined(separator: ","))
    }
}
...
class StrategyConceptual: XCTestCase {
    ...
    func test() {
        ...
        debugPrint("Client: Strategy is set to normal sorting.\n")
