...
class LoginViewController: BaseAuthViewController {
    ...
    func loginButtonSelected() {
        debugPrint("Login View Controller: User selected Login button")

        let request = LoginRequest(email: "smth@gmail.com", password: "123HardPass")

        if let error = handler?.handle(request) {
            debugPrint("Login View Controller: something went wrong")
            debugPrint("Login View Controller: Error -> " + (error.errorDescription ?? ""))
        } else {
            debugPrint("Login View Controller: Preconditions are successfully validated")
        }
    }
}
...
class SignUpViewController: BaseAuthViewController {
    ...
    func signUpButtonSelected() {
        debugPrint("SignUp View Controller: User selected SignUp button")
        ...
        if let error = handler?.handle(request) {
            debugPrint("SignUp View Controller: something went wrong")
            debugPrint("SignUp View Controller: Error -> " + (error.errorDescription ?? ""))
        } else {
            debugPrint("SignUp View Controller: Preconditions are successfully validated")
        }
    }
}
...
class ChainOfResponsibilityRealWorld: XCTestCase {
    ...
    func testChainOfResponsibilityRealWorld() {
        debugPrint("Client: Let's test Login flow!")
