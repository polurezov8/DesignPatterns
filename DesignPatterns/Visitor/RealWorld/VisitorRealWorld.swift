...
class VisitorRealWorld: XCTestCase {
    ...
    private func clientCode(handle notifications: [Notification], with policy: NotificationPolicy) {
        ...
        notifications.forEach { item in
            ...
            guard !item.accept(visitor: blackList) else {
                debugPrint("\tWARNING: " + item.description + " is in a black list")
                return
            }

            if item.accept(visitor: policy) {
                debugPrint("\t" + item.description + " notification will be shown")
            } else {
                debugPrint("\t" + item.description + " notification will be silenced")
            }
        }
    }
